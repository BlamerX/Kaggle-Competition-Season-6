"""
S6E9 V22 - XGBoost Hyperparameter Search on the Artifact Feature Matrix (GPU)
================================================================================
Strategy: keep V20's feature matrix exactly and tune the estimator on it for
          the first time, then confirm the leaders on a second CV split

Reference: V20 ran another team's XGBoost parameters (tuned for a 94-feature
set) against our 378 features — they were never fitted to our own matrix.

Device: GPU (cuda) | Est. Time: ~150-180 min

Search design (one run, no external files read):
1. Stage A - 6 configs x 5-fold KFold(rs=42), the canonical split shared with
   V19/V20 so the OOF stays comparable. Fold matrices are built once per fold
   and reused by every config, so the search costs training time only.
2. Stage B - the top 3 re-run on KFold(rs=7). One 5-fold OOF carries +/-0.00069
   noise, 10x the deltas we chase, so a config must hold up twice.
3. Winner = best mean OOF over the two splits. Config c0 is V20's own config run
   in the same job, so "delta vs c0" is the answer. sub_v22 averages the winner's
   test predictions over both splits; oof_v22 is its rs=42 OOF.
...
class CFG:
    ...
    # Search control
    SEARCH_SEED = 42     # canonical split, same as V19/V20
    CONFIRM_SEED = 7     # second split to confirm the top configs on
    TOP_K = 3            # how many configs advance to the confirmation stage
...
# =============================================================================
# 4. MODEL PARAMETERS — search grid
# =============================================================================
# XGB_BASE is V20's configuration (najiama's recipe) with a shorter budget:
# V20 converged between 3375 and 4984 trees, so 8000 with od 400 is
# effectively lossless and ~30% faster for a search that runs 6 configs.
XGB_BASE = {
...
# Each entry overrides XGB_BASE. The grid deliberately spans the two axes the
# diagnostics flagged: leaf capacity (how many splits a row can support) and
# column subsampling (how much each raw interaction can monopolise the gain).
XGB_CONFIGS = [
    {'name': "c0 V20 baseline",
     'params': {}},
    {'name': "c1 shallow + wide cols",
     'params': {'max_depth': 3, 'max_leaves': 8, 'gamma': 1.0,
                'colsample_bytree': 0.85}},
    {'name': "c2 deeper + heavy leaf reg",
     'params': {'max_depth': 6, 'max_leaves': 32, 'min_child_weight': 50.0,
                'colsample_bytree': 0.35, 'reg_lambda': 3.0}},
    {'name': "c3 sparse cols + high gamma",
     'params': {'max_depth': 5, 'max_leaves': 16, 'gamma': 6.0,
                'min_child_weight': 200.0, 'colsample_bytree': 0.25,
                'reg_lambda': 6.0, 'alpha': 2.0}},
    {'name': "c4 low min_child_weight (more splits)",
     'params': {'max_depth': 5, 'max_leaves': 32, 'min_child_weight': 1.0,
                'gamma': 0.5, 'colsample_bytree': 0.5}},
    {'name': "c5 tiny lr, near-depthless lossguide",
     # max_depth stays high so grow_policy=lossguide + max_leaves decides the
     # shape; max_depth=0 is degenerate in XGBoost and yields a constant model
     'params': {'max_depth': 12, 'max_leaves': 64, 'learning_rate': 0.008,
                'min_child_weight': 500.0, 'colsample_bytree': 0.6,
                'reg_lambda': 10.0}},
]
...
if __name__ == "__main__":
    ...
    print(f"Search: {len(XGB_CONFIGS)} configs on rs={CFG.SEARCH_SEED}, top-{CFG.TOP_K} confirmed on rs={CFG.CONFIRM_SEED}")
    print(f"Features: identical to V19/V20 — fixed digits + lift/novelty + structural flags")
    print(f"Change vs V20: estimator tuned on OUR matrix (V20 used foreign params)")
    ...
    # =========================================================================
    # [3/5] HYPERPARAMETER SEARCH (5-Fold CV, per-fold orig concat + Triple TE)
    # =========================================================================
    print(f"\n[3/5] XGBoost hyperparameter search ({CFG.N_FOLDS}-Fold CV, orig concat + Triple TE)...")
    print(f"   Stage A: {len(XGB_CONFIGS)} configs on KFold(rs={CFG.SEARCH_SEED}) — canonical split")
    ...
    def run_split(seed, cfg_indices):
        """
        Run one KFold split for the given config indices.

        The fold matrices (Triple TE) are built ONCE per fold and shared by every
        config, so the search costs training time only, never re-encoding.
        Returns (oof by cfg, test by cfg, best-iters by cfg, n_te_features).
        """
        names = ", ".join(XGB_CONFIGS[i]['name'] for i in cfg_indices)
        print(f"\n   ===== SPLIT rs={seed} =====")
        ...
        oof_by_cfg  = {ci: np.zeros(len(y)) for ci in cfg_indices}
        test_by_cfg = {ci: np.zeros(len(test_X)) for ci in cfg_indices}
        iters_by_cfg = {ci: [] for ci in cfg_indices}
        n_te = 0
        t_seed = time.time()
        ...
        for fold, ((train_idx, val_idx), (or_train_idx, or_val_idx)) in enumerate(
            ...
            # ---- Train every config on this fold's matrices ----
            for ci in cfg_indices:
                ...
                oof_by_cfg[ci][val_idx] = val_probs
                test_by_cfg[ci] += model.predict_proba(X_test_fold)[:, 1] / CFG.N_FOLDS

                best_iter = (model.best_iteration if hasattr(model, 'best_iteration')
                             and model.best_iteration is not None else params['n_estimators'])
                iters_by_cfg[ci].append(best_iter)

                fold_auc = auc_score(y_val.values, val_probs)
                print(f"         {cfg['name']:<40} AUC {fold_auc:.5f} | BestIter {best_iter}")

                if fold == 0 and ci == cfg_indices[0]:
                    imp = pd.DataFrame({
                        'feature': X_train.columns,
                        'importance': model.feature_importances_,
                    }).sort_values('importance', ascending=False)
                    print(f"\n         Top-10 feature importances ({cfg['name']}, fold 1):")
        ...
        for ci in cfg_indices:
            oof_auc = auc_score(y_np, oof_by_cfg[ci])
            print(f"   rs={seed} OOF | {XGB_CONFIGS[ci]['name']:<40} {oof_auc:.5f} | "
                  f"iters {iters_by_cfg[ci]}")

        return oof_by_cfg, test_by_cfg, iters_by_cfg, n_te

    # ---- Stage A: search on the canonical split ----
    oofA, testA, itersA, n_te = run_split(CFG.SEARCH_SEED, list(range(len(XGB_CONFIGS))))
    aucA = {ci: auc_score(y_np, oofA[ci]) for ci in oofA}

    print(f"\n{'='*78}")
    print("STAGE A RANKING — OOF on the canonical split (rs=%d)" % CFG.SEARCH_SEED)
    print(f"{'='*78}")
    rankedA = sorted(aucA.items(), key=lambda kv: -kv[1])
    for rank, (ci, a) in enumerate(rankedA):
        print(f"   {rank+1}. {XGB_CONFIGS[ci]['name']:<42} {a:.5f}  (delta vs c0 {a - aucA[0]:+.5f})")
    print(f"   Reference — V20 full-budget run: OOF 0.94606 | V19: 0.94599")

    top_k = [ci for ci, _ in rankedA[:CFG.TOP_K]]
    print(f"\n   Advancing to confirmation: {[XGB_CONFIGS[ci]['name'] for ci in top_k]}")

    # ---- Stage B: confirm the leaders on a second split ----
    oofB, testB, itersB, _ = run_split(CFG.CONFIRM_SEED, top_k)
    aucB = {ci: auc_score(y_np, oofB[ci]) for ci in oofB}
    ...
    combined = []
    for ci in top_k:
        mean_auc = 0.5 * (aucA[ci] + aucB[ci])
        combined.append((ci, aucA[ci], aucB[ci], mean_auc))
    combined.sort(key=lambda t: -t[3])
    for rank, (ci, a42, a7, ma) in enumerate(combined):
        print(f"   {rank+1}. {XGB_CONFIGS[ci]['name']:<42} rs{CFG.SEARCH_SEED} {a42:.5f} | "
              f"rs{CFG.CONFIRM_SEED} {a7:.5f} | mean {ma:.5f}")

    # ---- Winner ----
    winner_ci, w_a42, w_a7, w_mean = combined[0]
    print(f"\n   WINNER: {XGB_CONFIGS[winner_ci]['name']}")
    print(f"      params override: {XGB_CONFIGS[winner_ci]['params']}")

    # OOF reported on the canonical split (comparable with V19/V20);
    # test predictions average both splits -> variance reduction on the submission
    oof_probs  = oofA[winner_ci]
    test_probs = 0.5 * (testA[winner_ci] + testB[winner_ci])
    ...
    best_iters = itersA[winner_ci]

    print(f"\n   Winner OOF (rs={CFG.SEARCH_SEED}): {oof_cv:.5f}")
    print(f"   Winner OOF (rs={CFG.CONFIRM_SEED}): {w_a7:.5f}")
    print(f"   Mean of splits: {w_mean:.5f}  (V20 reference 0.94606)")
    print(f"   Baseline c0 on rs={CFG.SEARCH_SEED}: {aucA[0]:.5f}")
    ...
    # =========================================================================
    print(f"\n{'='*80}")
    print(f"V22 RESULTS — XGBoost Hyperparameter Search (GPU)")
    print(f"{'='*80}")
    print(f"Features: {len(FEATURES)} base + {n_te} Triple TE = {len(FEATURES) + n_te} total")
    print(f"Winner: {XGB_CONFIGS[winner_ci]['name']} | overrides: {XGB_CONFIGS[winner_ci]['params']}")
    print(f"Control c0 (V20 params) rs{CFG.SEARCH_SEED}: {aucA[0]:.5f}")
    print(f"Winner rs{CFG.SEARCH_SEED}: {w_a42:.5f} | rs{CFG.CONFIRM_SEED}: {w_a7:.5f} | mean: {w_mean:.5f}")
    print(f"Gain over c0 (rs={CFG.SEARCH_SEED}): {w_a42 - aucA[0]:+.5f}")
    ...
    print(f"Submission: winner's test predictions averaged over both CV splits")

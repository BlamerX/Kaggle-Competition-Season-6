"""
S6E9 V24 - CatBoost on the Generator-Artifact Feature Matrix (GPU)
================================================================================
Strategy: V19/V20's exact feature matrix (LB 0.94639) with the model family
          swapped to CatBoost — the one GBDT family never given the artifacts

Reference: our own V8 (CatBoost, 2026-09-09) reached OOF 0.94583 / LB 0.94603,
but that predates the generator-lift features and the digit fix — this family
has never seen our strongest representation.

Device: GPU (cuda) | Est. Time: ~25-40 min

Why the estimator, not the representation: every feature-side screen returned
+0.00000 incremental over V19 (extended lift variants, manifold distance,
propensities, a multivariate density ratio, subgroup recalibration), and V21
showed adding crosses actively hurts this model. CatBoost's ordered target
statistics and symmetric trees are the most different inductive bias left that
is not neural, and every neural family we tried finished below the trees.
...
The only change here is the model family: CatBoost replaces XGBoost.
"""
...
class CFG:
    VERSION_NAME = "v24"
    EXP_ID = "S6E9_V24_CatBoost_Artifacts"
...
# 4. MODEL PARAMETERS
# =============================================================================
# Depth 6 with strong L2 and Bernoulli sampling — CatBoost's own sweet spot on
# this data (V8 used Ordered boosting at depth 5 / lr 0.015 and reached
# OOF 0.94583 pre-artifacts). Plain (non-Ordered) boosting is used because every
# categorical column is already target-encoded to numeric by the pipeline.
CAT_PARAMS = {
...
if __name__ == "__main__":
    ...
    print(f"Model: CatBoost depth=6 GPU (task_type=GPU, od_wait=500)")
    print(f"Features: identical to V19/V20 — fixed digits + lift/novelty + structural flags")
    print(f"Change vs V20: model family only (XGBoost depth=4 -> CatBoost)")
    ...
    print(f"   Missing in orig: {int(orig_aligned.isna().sum().sum())} (CatBoost handles NaN natively)")
    ...
    for fold, ((train_idx, val_idx), (or_train_idx, or_val_idx)) in enumerate(
        ...
        # Any categorical left over is dropped; the pipeline encodes everything numerically
        ...
        best_iter = best_iter if best_iter is not None else CAT_PARAMS['iterations']
    ...
    # =========================================================================
    print(f"\n{'='*80}")
    print(f"V24 RESULTS — CatBoost + Generator-Artifact Features (GPU)")
    print(f"{'='*80}")
    print(f"Features: {len(FEATURES)} base + {len(te_feature_names)} Triple TE = {len(FEATURES) + len(te_feature_names)} total")
    print(f"Model: CatBoost depth=6, lr=0.03, l2_leaf_reg=3.0, subsample=0.8 (Bernoulli)")
    ...
    print(f"Reference — V20 XGBoost 0.94606 / V19 LightGBM 0.94599 / V8 CatBoost 0.94583")

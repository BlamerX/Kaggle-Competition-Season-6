"""
S6E9 V3 - LightGBM Baseline (CPU)
================================================================================
Strategy: LightGBM with Multi-Scale Smooth Keys + Triple Target Encoding
          + Original Dataset Target Means + Synthetic-Artifact Flags
          + Digit Features + Engineered Interactions + Original Data (per-fold concat)

Reference: https://www.kaggle.com/code/najiama/pure-lgbm-model-cv-0-94606-lb-0-94637
- CV 0.94607 / LB 0.94638 (best public single LightGBM)
- Using 5-fold StratifiedKFold

Device: CPU ONLY (no GPU, no cuDF)
- Reference notebook was trained on CPU; faithful port matches reference exactly
- LightGBM GPU mode has known issues with colsample_bytree=0.3 and max_bin=1024
- Run on Kaggle with accelerator OFF (CPU only) for cleanest reproduction

Key Techniques from Reference (proven CV 0.94607 / LB 0.94638):
1. Triple Target Encoding (auto, 10, 100 smoothing) on ALL cat + num-as-string cols
   - smooth=100.0 (heavy Bayesian prior) keeps micro-bins from overfitting
   - Applied per-fold to prevent leakage
2. Multi-Scale "Smooth Keys" (Markus's technique) — 4 new string columns:
   - income_exact_int, income100_floor, income1000_floor, commute_integer
   - Coarse-to-fine bins that get TE'd at 3 smoothing levels
3. Original dataset target means — static lookup table per cat+num column
4. Synthetic-artifact magic flags (4 community-validated):
   - is_30k_spike (income == 30000, mode collapse)
   - is_millionaire_cliff (income >= 170537, 100% buy rate)
   - is_dead_zone (income in [38000, 42000], 0% buy rate)
   - is_env_hater (ECL == 1)
5. Numeric->string conversion + frequency encoding on all cats
6. Digit features (8 per numeric column, positions -4 to 3)
7. Feature selection: drop constant + perfectly-correlated (corr=1.0)
8. Drops Number_of_Cars_Owned (low signal per reference)

Kept from our V1/V2 (proven signals, no conflict with reference):
...
Dropped from V2 (V2 proved low ROI or cannibalizes):
- _recipe_score (cannibalized _ECL_x_Subsidy in V2)
- Single TE on all features (replaced by Triple TE on cat+num-as-string cols)
- Count encoding (replaced by true frequency encoding)
- V2's XGBoost hyperparameters (replaced by LGBM params)

LightGBM Parameters (EXACT from reference notebook):
  ...
  - feature_pre_filter=False, metric='auc', n_jobs=-1, device='cpu'
...
Expected: OOF ~0.9460-0.9465, LB ~0.9464 (matches reference's 0.94638)
Golden Rules: SKF(5, shuffle=True, rs=42), AUC metric, raw OOF for hill climber
"""

# =============================================================================
# 1. IMPORTS (CPU ONLY — no cuDF, no GPU dependencies)
# =============================================================================
import os
import gc
import time
...
# =============================================================================
# 4. MODEL PARAMETERS (EXACT from reference notebook — CV 0.94607, LB 0.94638)
# =============================================================================
# LightGBM with leaf-wise growth (num_leaves controls complexity, not max_depth)
# Key differences from V2 XGBoost:
# - max_depth=5 (shallower than XGB's 7; LGB uses num_leaves=32 for complexity)
# - colsample_bytree=0.3 (very aggressive feature subsampling — 3x more than XGB's 0.9)
# - learning_rate=0.02 (between V1's 0.05 and V2's 0.005)
# - n_estimators=20000 (highest yet; early stopping at 500 rounds patience)
# - device='cpu' (explicit; reference notebook used CPU)
...
LGBM_PARAMS = {
    ...
    'n_jobs': -1,
    'verbose': -1,
    'device': 'cpu',  # Explicit CPU; reference notebook used CPU
}
...
def add_engineered_features(df):
    """
    EDA-validated engineered features (kept from V1/V2 — proven signals).
    ...
    NOTE: _recipe_score from V2 is DROPPED (V2 proved it cannibalizes _ECL_x_Subsidy).
    """
    df = df.copy()
...
def add_numeric_as_string(df, num_cols):
    ...
    Reference notebook's approach: gives LGBM a different view of numerics.
...
if __name__ == "__main__":
    ...
    # Reference notebook drops Number_of_Cars_Owned (low signal)
    # We KEEP it for per-fold concat but note it's low priority
    ...
    print(f"   Missing in orig: {orig_aligned.isna().sum().sum()} (LGBM handles NaN natively)")
    ...
    # 2b. Engineered interactions + log transforms + hard-edge flags (from V1/V2)
    ...
    for col in CATS + NUMS:
        ...
        if org_mean_name in train.columns and col in orig_aligned.columns:
            # For orig, map each value to its own target mean (slightly leaky but
            # orig is concat'd into train only, never used as validation)
            ...
            # Column not in orig_aligned (e.g., digit features) — fill with global mean
    ...
    # Get all numeric columns currently in train (excluding target)
    ...
    # Also include our engineered numeric features
    ...
    # Build list of columns to frequency-encode (must exist in both train and test)
    freq_target_cols = CATS + train_num_cat_cols + [
        'income_exact_int', 'income100_floor', 'income1000_floor', 'commute_integer'
    ]
    # Filter to columns that exist in both train and test
    ...
    # Apply same freq maps to orig (using train+test maps; unknown values get 0)
    # Note: orig's freq features will be computed in the training loop per-fold
    # since orig is concat'd per-fold. For now, add placeholder columns.
    ...
    # Drop same columns from orig_aligned
    ...
    for fold, ((train_idx, val_idx), (or_train_idx, or_val_idx)) in enumerate(
        ...
        # Reference notebook's approach: 3 smoothings give model different views
        # Applied per-fold to prevent leakage
        te_feature_names = []
        ...
        # Keep only columns that are NOT in TARGET_ENCODE_COLS
        cols_to_drop_after_te = [c for c in TARGET_ENCODE_COLS if c in X_train.columns]
        X_train = X_train.drop(columns=cols_to_drop_after_te)
        X_val   = X_val.drop(columns=cols_to_drop_after_te)
        X_test_fold = X_test_fold.drop(columns=cols_to_drop_after_te)

        if fold == 0:
            print(f"      Final feature count: {len(X_train.columns)}")

        # ---- Train LightGBM (reference notebook's exact params, CPU) ----
    ...
    print(f"   Mean +/- std: {np.std(fold_scores):.5f} +/- {np.std(fold_scores):.5f}")
    ...
    # =========================================================================
    print(f"\n{'='*80}")
    print(f"V3 RESULTS — LightGBM Baseline ({CFG.DEVICE})")
    print(f"{'='*80}")
    ...
    print(f"Hyperparameters: lr=0.02, depth=5, num_leaves=32, colsample=0.3 (reference notebook exact, CPU)")

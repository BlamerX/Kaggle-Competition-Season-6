"""
S6E9 V8 - CatBoost Ordered Boosting (GPU)
================================================================================
Strategy: CatBoost with Ordered Boosting + V3 FE Pipeline + Native cat_features

Reference: S6E2 V39 Ordered Boosting — best CatBoost on S6E2 (0.95390 LB)
- S6E2 V39 used boosting_type='Ordered' — prevents target leakage natively
- Our V4 used default 'Plain' boosting — scored 0.94590

Key difference from V4 CatBoost:
1. boosting_type='Ordered' (V4: 'Plain') — THE key change
2. bootstrap_type='Bernoulli' (V4: 'Bayesian')
3. auto_class_weights='Balanced' (V4: none)
4. learning_rate=0.015 (V4: 0.005)
5. depth=5 (V4: 7)
6. l2_leaf_reg=5.0 (V4: 2.0)
7. random_strength=1.5 (V4: not set)
8. subsample=0.8 (V4: not set)

Why Ordered Boosting might help:
- Ordered boosting uses permutations to prevent prediction shift
- Each tree is built on residuals computed without using the current object
- This reduces overfitting on synthetic data (S6E9 is synthetic)
- S6E2: Ordered (0.95390) > Plain (0.95385) — marginal but real

Kept from V4 (proven on S6E9):
- V3's full FE pipeline (344 features: digit, interactions, smooth keys, magic flags, etc.)
- Native cat_features handling (10 string columns)
- Triple TE (auto, 10, 100) on 63 cols
...
class CFG:
    VERSION_NAME = "v8"
    EXP_ID = "S6E9_V8_CatBoost_Ordered"
    DEVICE = "GPU"
...
# =============================================================================
# 4. MODEL PARAMETERS (S6E2 V39 Ordered Boosting — proven 0.95390 on S6E2)
# =============================================================================
# CatBoost with Ordered boosting (prevents prediction shift)
# Key difference from V4: boosting_type='Ordered' (V4 used default 'Plain')
#
# S6E2 V39 params:
#   iterations=8000, learning_rate=0.015, depth=5
#   l2_leaf_reg=5.0, random_strength=1.5
#   boosting_type='Ordered' (KEY CHANGE)
...
CATBOOST_PARAMS = {
    ...
    'l2_leaf_reg': 5.0,
    'random_strength': 1.5,
    'boosting_type': 'Ordered',      # KEY: Ordered instead of Plain
    'bootstrap_type': 'Bernoulli',   # Required for Ordered boosting
    'subsample': 0.8,
    'eval_metric': 'AUC',
    'auto_class_weights': 'Balanced',
    'early_stopping_rounds': 200,
...
if __name__ == "__main__":
    ...
    print(f"Device: {CFG.DEVICE} (CatBoost Ordered) | Folds: {CFG.N_FOLDS}")
    print(f"Key Change: boosting_type='Ordered' (V4 used 'Plain')")
    print(f"Reference: S6E2 V39 Ordered Boosting (0.95390)")
    print("="*80)
    ...
    # =========================================================================
    print(f"\n[3/5] Training CatBoost Ordered ({CFG.N_FOLDS}-Fold CV, orig concat + Triple TE)...")
    ...
    for fold, ((train_idx, val_idx), (or_train_idx, or_val_idx)) in enumerate(
        ...
        # Keep original CATS + smooth keys for CatBoost native handling
        ...
        # Ensure all cat_features are strings
    ...
    # =========================================================================
    print(f"\n{'='*80}")
    print(f"V8 RESULTS — CatBoost Ordered Boosting ({CFG.DEVICE})")
    print(f"{'='*80}")
    print(f"Features: {len(FEATURES)} base + {len(te_feature_names)} Triple TE = {len(FEATURES) + len(te_feature_names)} total")
    print(f"  - CatBoost native cat_features: {len(cat_features)}")
    print(f"Key change from V4: boosting_type='Ordered' (V4 used 'Plain')")
    print(f"Hyperparameters: lr=0.015, depth=5, l2_leaf_reg=5.0, Ordered, Bernoulli, Balanced")

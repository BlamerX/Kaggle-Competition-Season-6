"""
S6E9 V27 - XGBoost Structural Ablation: Original-Row Prior Weight + Pairwise Loss (GPU)
================================================================================
Strategy: test the two structural knobs every version has silently assumed —
          how much the original 10k rows count, and whether we optimise the
          metric we are scored on

Reference: V20/V23 (XGBoost depth=4 lossguide, OOF 0.94606 / LB 0.94641) and the
V22/V25/V26 ablation harness. V25 proved the format: one build per fold, an
in-run control that must reproduce the previous version, single-factor arms, and
a paired DeLong gate computed from this run's own OOF vectors.

Device: GPU (cuda) | Est. Time: ~200-280 min

Arms (each changes exactly one factor vs a0; a0 must reproduce V20's 0.94606):
  a0 control          = V20's config, comp + original rows concatenated at weight 1
  a1 drop originals   = a0 minus the 10k original rows entirely
  a2 originals x10    = a0 but the original rows carry sample weight 10
  a3 rank:pairwise    = a0's model, objective swapped to the pairwise AUC surrogate
  a4 pairwise + x10   = a3 with the original rows replicated 10x

Why these two:
1. ORIGINAL-ROW WEIGHT. Every version since V1 concatenates the 10k original
   rows, which are 1.8% of the training matrix and whose labels are the clean
   recipe while the competition labels are recipe + generator noise. Upweighting
   them trains on cleaner supervision; dropping them tests whether the concat
   earns its place at all (the reference 0.94639 recipe does not use them).
2. PAIRWISE LOSS. ROC AUC is a pairwise ranking metric and we have optimised
   pointwise logloss for 26 versions. rank:pairwise maximises a surrogate built
   from exactly the row pairs AUC counts. Rows are shuffled into random groups
   of 64, so the pairs a group exposes are a uniform subsample of the global
   positive-negative pairs the metric averages over.

Implementation notes:
- Booster API (DMatrix + xgb.train) throughout: needed for `weight` and
  `set_group`, and it is what lets one code path serve all five arms.
- This XGBoost build rejects `weight` together with `set_group`, so a4 expresses
  the weight by REPLICATING the original rows 10x. In a pairwise loss a repeated
  row contributes each of its pairs once per copy, i.e. duplication and weight k
  are the same objective, so a4 stays a single-factor change from a3.
- rank:pairwise emits ranking scores, not probabilities. XGBoost accepts
  eval_metric='auc' for it, so early stopping watches the real metric; the
  submission of a ranking winner holds monotone scores, which AUC is invariant
  to. The results block flags this if it happens.
- OOF stays purely out-of-fold. Stage B (rs=7) runs ONLY if an arm clears the
  z > 3 gate, so a negative answer costs one split.
"""
...
class CFG:
    VERSION_NAME = "v27"
    EXP_ID = "S6E9_V27_XGB_StructuralAblation"
    ...
    # Training budget: V20 converged between 3375 and 4984 trees, and V22 showed
    # a 12000/500 budget reproduces its fold AUCs exactly, so this is lossless
    # while capping how long a slow (pairwise) arm can run.
    NUM_ROUND = 12000
    ES_ROUNDS = 500

    # Original-row supervision (arms a2 / a4)
    ORIG_WEIGHT = 10.0        # a2 sample weight
    ORIG_REPEAT = 10          # a4 replication count (the pairwise-loss equivalent)

    # rank:pairwise grouping (arms a3 / a4)
    RANK_GROUP_SIZE = 64      # ~11 positives per group at pos rate 0.1746
...
# =============================================================================
# 4. MODEL PARAMETERS — V20 control and the structural arms
# =============================================================================
XGB_BASE = {
...
# orig: 'concat' (weight 1) | 'drop' | 'upweight' | 'repeat'
ARMS = [
    {'name': "a0 control (V20 config)",          'orig': "concat",   'rank': False},
    {'name': "a1 no original rows",              'orig': "drop",     'rank': False},
    {'name': "a2 original rows weight x10",      'orig': "upweight", 'rank': False},
    {'name': "a3 rank:pairwise objective",       'orig': "concat",   'rank': True},
    {'name': "a4 rank:pairwise + orig x10 reps", 'orig': "repeat",   'rank': True},
]
...
def random_groups(n_rows, group_size, seed):
    """
    Shuffle rows into contiguous random groups and return (permutation, sizes).

    Groups must be contiguous for DMatrix.set_group. Because the assignment is
    uniform at random, the within-group positive/negative pairs are a uniform
    subsample of the global pairs ROC AUC averages over, and every group holds
    ~group_size * 0.1746 positives, so pair signal is never starved.
    """
    perm = np.random.RandomState(seed).permutation(n_rows)
    group_of_perm = np.arange(n_rows) // group_size
    order = perm[np.argsort(group_of_perm, kind='stable')]
    sizes = np.bincount(np.arange(n_rows) // group_size)
    return order, sizes.astype('int64')
...
if __name__ == "__main__":
    ...
    print(f"Probe 1: how much do the 10k original rows earn? (drop / x{CFG.ORIG_WEIGHT:g} weight)")
    print(f"Probe 2: does optimising the pairwise AUC surrogate beat pointwise logloss?")
    print(f"Control a0 = V20's config, so a0 must land on V20/V23's OOF 0.946060")
    print(f"Arms: {', '.join(a['name'] for a in ARMS)}")
    print(f"Original data: per-fold concat, Buyer_ID dropped; rank arms group rows by {CFG.RANK_GROUP_SIZE}")
    ...
    # =========================================================================
    # [3/5] STRUCTURAL ABLATION (5-Fold CV, per-fold orig concat + Triple TE)
    # =========================================================================
    print(f"\n[3/5] Structural ablation ({CFG.N_FOLDS}-Fold CV, orig concat + Triple TE)...")
    ...
    def run_split(seed, arm_indices):
        ...
        arm; arms differ only in which rows they train on and which objective they
        optimise, so the ablation costs training time only, never re-encoding.
        """
        ...
        # Fold 1 is the canary: if this XGBoost/CUDA build refuses an objective, the
        # arm is dropped for the whole split instead of aborting a multi-hour run.
        dead = {}
        n_te = 0
        t_seed = time.time()
        ...
        for fold, ((train_idx, val_idx), (or_train_idx, or_val_idx)) in enumerate(
            ...
            y_comp = y_full[:n_comp]
            y_orl  = y_full[n_comp:]
            A_comp = A_full[:n_comp]
            A_orl  = A_full[n_comp:]
            ...
            for ai in arm_indices:
                ...
                # 1. which supervision rows, and with what influence
                if arm['orig'] == 'drop':
                    Xa, ya = A_comp, y_comp
                    wa = None
                    w_note = "orig dropped"
                elif arm['orig'] == 'upweight':
                    Xa = A_full
                    ya = y_full
                    wa = np.ones(len(ya), dtype='float32')
                    wa[n_comp:] = CFG.ORIG_WEIGHT
                    w_note = f"orig weight x{CFG.ORIG_WEIGHT:g}"
                elif arm['orig'] == 'repeat':
                    reps = [A_orl] * CFG.ORIG_REPEAT
                    Xa = np.concatenate([A_comp] + reps, axis=0)
                    ya = np.concatenate([y_comp] + [y_orl] * CFG.ORIG_REPEAT)
                    wa = None
                    w_note = f"orig repeated x{CFG.ORIG_REPEAT}"
                else:
                    Xa, ya, wa = A_full, y_full, None
                    w_note = "orig weight 1"

                # 2. objective, and the row ordering / groups a pairwise loss needs
                params = dict(XGB_BASE)
                order = np.arange(len(ya))
                gr = None
                if arm['rank']:
                    params['objective'] = 'rank:pairwise'
                    order, gr = random_groups(len(ya), CFG.RANK_GROUP_SIZE, seed * 100 + fold)
                    Xa = np.ascontiguousarray(Xa[order])
                    ya = ya[order]
                    if wa is not None:
                        wa = wa[order]

                try:
                    dtrain = xgb.DMatrix(Xa, label=ya, weight=wa, feature_names=cols_fold)
                    if gr is not None:
                        dtrain.set_group(gr)

                    dva = xgb.DMatrix(A_val, label=y_val_np, feature_names=cols_fold)
                    if gr is not None:
                        _, gr_val = random_groups(len(y_val_np), CFG.RANK_GROUP_SIZE, seed * 1000 + fold)
                        dva.set_group(gr_val)
                    ...
                    # A fold-1 failure is a platform limitation, not a bug in the arm:
                    # drop the arm and keep the run. Later folds must not be masked.
                    if fold > 0:
                        raise
                ...
                print(f"         {arm['name']:<34} AUC {fold_auc:.5f} | trees {best_iter} | {w_note}")
            ...
            del A_full, A_val, A_test, A_comp, A_orl
            gc.collect()
        ...
        return oof_by_arm, test_by_arm, iters_by_arm, n_te, z_by_arm, dead

    # ---- Stage A: all arms on the canonical split ----
    oofA, testA, itersA, n_te, zA, deadA = run_split(CFG.SEARCH_SEED, list(range(len(ARMS))))
    if 0 in deadA:
    ...
    print(f"   Reference — V20/V23 stored OOF: 0.946060 | V22 0.946076 | V25 0.946094 | V19 0.945987")
    ...
    if passed:
        ...
        oofB, testB, itersB, _, zB, deadB = run_split(CFG.CONFIRM_SEED, top_k)
        aucB = {ai: auc_score(y_np, oofB[ai]) for ai in oofB if ai not in deadB}
        print(f"\n{'='*78}")
    ...
    if winner_arm['rank']:
        print(f"   NOTE: the winner is a ranking learner — sub_v{CFG.VERSION_NAME[1:]} holds "
              f"monotone ranking scores, not probabilities (AUC is invariant to that).")
    ...
    # =========================================================================
    print(f"\n{'='*80}")
    print(f"V27 RESULTS — XGBoost Structural Ablation (GPU)")
    print(f"{'='*80}")
    print(f"Features: {len(FEATURES)} base + {n_te} Triple TE = {len(FEATURES) + n_te} total")
    print(f"Control a0 (V20 config) rs{CFG.SEARCH_SEED}: {aucA[0]:.5f}  "
          f"(V20/V23's stored OOF is 0.946060)")
